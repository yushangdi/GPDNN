# MODIFY FROM https://github.com/rhaps0dy/convnets-as-gps
...
def conv2d_fixed_padding(inputs, var, kernel_size, strides,
    ...
    with tf.name_scope(name):
        ...
        W = tf.fill([kernel_size, kernel_size, C, 1], tf.cast(var,tf.float32)/tf.cast(fan_in,tf.float32))
        if data_format == "NCHW":
...
#           [5,5,5]
        for i, num_blocks in enumerate(self.block_sizes):
            with tf.name_scope("block_layer_{}".format(i+1)):
                ...
                for j in range(1, num_blocks):
                    block_layers.append(inputs)
                    inputs = self.dense_block(inputs,1,apply_recurse_kern,block_layers[:-1])
    ...
    @gpflow.decors.params_as_tensors
    @gpflow.decors.name_scope()
    def dense_block(self,inputs,strides,apply_recurse_kern,block_layers):
        inputs = apply_recurse_kern(inputs)
        inputs = conv2d_fixed_padding(
            inputs=inputs, var=self.var_weight, kernel_size=3, strides=strides,
            data_format=self.data_format)
