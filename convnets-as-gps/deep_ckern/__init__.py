# https://github.com/rhaps0dy/convnets-as-gps
...
#                 f.write("***************")
#                 f.write("\n")
#                 f.write("{} {}".format("The shape for x is ", x.shape))
            a = tf.nn.convolution(
    ...
    @gpflow.decors.params_as_tensors
    @gpflow.decors.name_scope()
    def get_Wb(self, i, X_shape=None, n_samples=None, n_filters=None, pooling = False):
        ...
        if pooling == True:
            self._W[i] = tf.ones(shape_W, name="W_{}".format(i), dtype=settings.float_type)
            self._b[i] = tf.zeros(shape_b, name="b_{}".format(i), dtype=settings.float_type)
    ...
    def equivalent_BNN(self, X, n_samples, n_filters=128):
        ...
        for i in range(self.n_layers):
            ...
            if i in pooling_layers:
                ...
                is_pooling = False
            W, b = self.get_Wb(i, X.shape, n_samples, n_filters, is_pooling)
